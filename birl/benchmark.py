...
class ImRegBenchmark(Experiment):
    ...
    COL_PERCENTILE_THRESHOLD = 'Percentile threshold'

    # list of columns in cover csv
    COVER_COLUMNS = (COL_IMAGE_REF, COL_IMAGE_MOVE, COL_POINTS_REF, COL_POINTS_MOVE)
    COVER_COLUMNS_EXT = tuple(list(COVER_COLUMNS) + [COL_FULL_SCALE_MAGNIFICATION, COL_PIXEL_SIZE, COL_TARGET_IMAGE_SIZE, COL_SOURCE_IMAGE_SIZE, COL_TARGET_IMAGE_DIAGONAL, COL_SOURCE_IMAGE_DIAGONAL, COL_INITIAL_ROTATION, COL_PERCENTILE_THRESHOLD])
    ...
    def _visual_image_move_warp_lnds_move_warp(cls, item, path_dataset=None, path_experiment=None):
        ...
        fig = draw_images_warped_landmarks(image_target=image_ref, image_source=image_move, points_init=points_move, points_target=points_ref, points_warped=points_warp)
        return fig
    ...
    def _visual_image_move_warp_lnds_ref_warp(cls, item, path_dataset=None, path_experiment=None):
        ...
        # visualise the landmarks move during registration
        fig = draw_images_warped_landmarks(image_target=image_move, image_source=image_ref, points_init=points_ref, points_target=points_move, points_warped=points_warp)
        return fig
